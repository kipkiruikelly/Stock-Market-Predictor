"""
engines/paper_trading/engine.py
Paper trading engine logic. Extracted from paper_engine.py.
...
suggests placing, or connects to real-money orders.
...
unaffected); a real id is that user's own isolated portfolio.
"""
...
def asset_class(ticker: str) -> str:
    ...
    if t in FOREX:      return "forex"
    if t in COMMODITY:  return "commodity"
    if t in INDEX:      return "index"
...
def compute_metrics(closed_pnls: list, equity_series: list,
    ...
    """Performance metrics from closed-trade P&Ls and an equity time series.
    Below min_trades closed trades, returns {\"sufficient\": False, ...}
    and no ratio metrics, honestly."""
    import numpy as np
    n = len(closed_pnls)
...
def digest_summary(db) -> str:
    """One line for the staff daily digest."""
    from models import PaperTrade
...
def alpha_signals(db, cfg: dict, quotes: dict) -> dict:
    ...
    ranked, filtered by Pyth oracle confidence."""
    ...
    for t, score in scores.items():
        ...
        q    = quotes.get(t) or {}
        mult = pyth_confidence_multiplier(q.get("conf_pct"), cfg["pyth_wide_conf_pct"])
        final = blended * mult
...
def ml_signals(cfg: dict) -> dict:
    ...
    trained daily models produce non-HOLD actions."""
...
def _run_owner_cycle(db, cfg, quotes, sig_ml, sig_alpha, user_id=None):
    """One exits+entries+snapshots pass for a single owner."""
